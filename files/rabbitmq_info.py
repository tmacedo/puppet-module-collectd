...
def get_stats():
    ...
    try:
        p = subprocess.Popen([RABBITMQCTL_BIN, '-q', '-p', VHOST,
            'list_queues', 'name', 'messages', 'memory', 'consumers', 'messages_ready', 'messages_unacknowledged'],
    ...
    for line in p.stdout.readlines():
        ...
        stats['ctl_messages_ready_%s' % queue_name] = ctl_stats[4]
        stats['ctl_messages_unacknowledged_%s' % queue_name] = ctl_stats[5]
